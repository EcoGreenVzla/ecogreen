...
// --- Icon Components (Se mantienen para el Lightbox de la galería) ---
...
// --- Data for Gallery (Se mantiene de la versión previa) ---
const galleryImages = [
...
// --- Componentes Internos de Galería ---
...
const ImageCard: React.FC<{ src: string; onClick: () => void }> = ({ src, onClick }) => (
    ...
    >
        <div className="h-48 overflow-hidden">
            <img src={src} alt="Proyecto de canalización" className="w-full h-full object-cover transition-transform duration-500 ease-in-out group-hover:scale-110" />
        </div>
    </motion.div>
);
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 bg-black/90 flex flex-col items-center justify-center p-4" onClick={onClose}>
        <div className="relative w-full h-full flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            ...
            <AnimatePresence initial={false}>
                <motion.img key={currentIndex} src={images[currentIndex]} initial={{ opacity: 0, x: 50 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -50 }} className="max-h-[85vh] w-auto object-contain mx-auto" />
            </AnimatePresence>
            <button onClick={onPrev} className="absolute left-2 md:left-8 p-2 rounded-full bg-white/10 text-white"><ChevronLeftIcon className="w-8 h-8" /></button>
            <button onClick={onNext} className="absolute right-2 md:right-8 p-2 rounded-full bg-white/10 text-white"><ChevronRightIcon className="w-8 h-8" /></button>
        </div>
    </motion.div>
);
...
const Canalizaciones: React.FC = () => {
  ...
  return (
    <div className="w-full m-0 p-0">
      ...
      {/* 1. HERO SECTION: Conexión con HeroSliderResponsive */}
      <HeroSliderResponsive data={sliderData['canalizaciones']} />
                ...
                {isExpanded && (
                    <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden text-left">
                        <div className="space-y-4 text-lg text-gray-700 leading-relaxed pt-4 pb-8 border-t border-gray-200 mt-4">
                           {/* ... Texto completo de canalizaciones extraído del HTML ... */}
                           <p>El conocimiento empírico del funcionamiento de los canales se remonta a varios milenios...</p>
                        </div>
                    </motion.div>
                )}
            </AnimatePresence>
