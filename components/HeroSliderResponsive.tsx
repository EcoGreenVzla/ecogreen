...
const HeroSliderResponsive: React.FC<Props> = ({ data, autoPlayDuration = 8000 }) => {
  ...
  const theme = {
    ...
    mobile: {
      ...
      arrows: { offset: '10px', width: '25px', height: '45px', background: 'rgba(0, 0, 0, 0.3)', iconSize: '24px', iconThickness: '3px', zIndex: 95 }
    }
  };
  ...
  return (
    <div 
      ref={containerRef} 
      className={`relative z-0 w-full bg-white overflow-hidden ${isDragging ? 'cursor-grabbing' : 'cursor-grab'}`}
